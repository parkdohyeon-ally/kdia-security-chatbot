...
GEN3_PROCEDURE_KEYWORDS = {
    "수출승인": ["수출승인", "수출 승인", "R&D 자금 지원", "정부지원 받은"],
    "수출신고": ["수출신고", "수출 신고", "독자 개발", "R&D 자금 지원받지 않은"],
...
def detect_gen1_business_type(question: str):
    scores = {}
    for bt, kws in GEN1_BUSINESS_KEYWORDS.items():
        s = sum(1 for kw in kws if kw in question)
        if s > 0:
            scores[bt] = s
...
def detect_gen1_process(question: str):
    scores = {}
    for proc, kws in GEN1_PROCESS_KEYWORDS.items():
        s = sum(1 for kw in kws if kw in question)
        if s > 0:
            scores[proc] = s
    return max(scores, key=scores.get) if scores else None
...
def detect_gen2_stage(question: str):
    scores = {}
    for stage, kws in GEN2_STAGE_KEYWORDS.items():
        s = sum(1 for kw in kws if kw in question)
        if s > 0:
            scores[stage] = s
    return max(scores, key=scores.get) if scores else None
...
def detect_gen2_item(question: str):
    scores = {}
    for item, kws in GEN2_ITEM_KEYWORDS.items():
        s = sum(1 for kw in kws if kw in question)
        if s > 0:
            scores[item] = s
    return max(scores, key=scores.get) if scores else None
...
def detect_gen3_chapter(question: str):
    scores = {}
    for ch, kws in GEN3_CHAPTER_KEYWORDS.items():
        s = sum(1 for kw in kws if kw in question)
        if s > 0:
            scores[ch] = s
    return max(scores, key=scores.get) if scores else None
...
def detect_gen3_domain(question: str):
    scores = {}
    for dom, kws in GEN3_DOMAIN_KEYWORDS.items():
        s = sum(1 for kw in kws if kw in question)
        if s > 0:
            scores[dom] = s
    return max(scores, key=scores.get) if scores else None
...
def detect_gen3_risk_level(question: str):
    scores = {}
    for lv, kws in GEN3_RISK_LEVEL_KEYWORDS.items():
        s = sum(1 for kw in kws if kw in question)
        if s > 0:
            scores[lv] = s
    return max(scores, key=scores.get) if scores else None
...
def detect_gen3_measure_type(question: str):
    scores = {}
    for mt, kws in GEN3_MEASURE_TYPE_KEYWORDS.items():
        s = sum(1 for kw in kws if kw in question)
        if s > 0:
            scores[mt] = s
    return max(scores, key=scores.get) if scores else None
...
def detect_gen3_procedure(question: str):
    scores = {}
    for proc, kws in GEN3_PROCEDURE_KEYWORDS.items():
        s = sum(1 for kw in kws if kw in question)
        if s > 0:
            scores[proc] = s
    return max(scores, key=scores.get) if scores else None
